...
} from "../email-templates/authentication.templates.js";
...
import {
  createEncryptedJWT,
  decryptedEJWT,
} from "../utils/encrypted-payloads.js";
...
const userServices = {
  async sendSignupOtpToEmail(email) {
    ...
    if (user)
      throw new createError.Conflict(
        "A user with current email already exists"
      );
  ...
  async sendSignupOtpToPhone(phone) {
    ...
    if (user)
      throw new createError.Conflict(
        "A user with current phone number already exists"
      );
  ...
  async registerUserAccount(email, phone, firstName, lastName, location, dob) {
    await verifiedCredentialsServices.isEmailVerified(email);
  ...
  async getUserProfile(userId) {
    ...
    if (!user)
      throw new createError.NotFound(
        "User with given information doesn't exist"
      );
  ...
  async signInAccount(email, password) {
    ...
    if (!user.password)
      throw new createError.BadRequest(
        "User's password is not set, please create one first"
      );

    let passwordMatched = await bcrypt
  ...
  async getMyProfile(userId) {
    ...
    if (!user) throw new createError.NotFound("User doesn't exist found");
